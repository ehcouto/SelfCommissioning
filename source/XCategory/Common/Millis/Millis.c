 ...
 *
 * @details     Each timer can measure up to about 24.8 days.
 ...
 *                  MILLIS_TIMER_TYPE my_timer = Millis__BuildTimer();
 ...
 *
 *              Usage Example 4: Restore a non-volatile timer (on non-ACU project types)
...
//! A mask to limit the Milliseconds free-running counter to 31 bits.
#define MILLISECONDS_MASK 0x7FFFFFFF
...
void Millis__Initialize(void)
{
    #if ((MILLIS_SUPPORT_NV_TIMERS == ENABLED) && (SYSTEM_ACU_HANDLING == DISABLED))
        Init_Complete = FALSE;
        Millis_Store_Timer = Millis__BuildTimer();
        Milliseconds = 0;
    #endif
...
void Millis__SlowHandler(void)
{
        ...
        if (Init_Complete)
        {
            if (Millis__ReachedTargetSeconds(&Millis_Store_Timer, MILLIS_SAVE_PERIOD_SECONDS))
            {
                ...
                Millis__SubtractSeconds(&Millis_Store_Timer, MILLIS_SAVE_PERIOD_SECONDS);
            }
        }
        else
        {
...
//---------------------------------------------------------------------------------------------------------------------
/**
 * Build a timer and return it to the calling application.
 * @return A new paused timer with a value of 0 milliseconds
 */
MILLIS_TIMER_TYPE Millis__BuildTimer(void)
{
    MILLIS_TIMER_TYPE timer = { 0, 0 };
    return timer;
}
...
void Millis__Subtract(MILLIS_TIMER_TYPE* timer, uint32 milliseconds)
{
    if (timer != NULL)
    {
        ...
        {
            if (timer->Reference > milliseconds)
            {
                timer->Reference = timer->Reference - milliseconds;
            }
            else
...
{
    Millis__Subtract(timer, seconds * 1000);
}
...
{
    uint32 milliseconds = ConvertHmsToMilliseconds(hms);
    Millis__Subtract(timer, milliseconds);
}
