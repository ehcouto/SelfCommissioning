...
BOOL_TYPE Reveal__QueueCommand(REVEAL_COMMAND_TYPE * command)
{
        ...
        (IsMessageBlocked(command->api, command->opcode) == TRUE))))
    {
            ...
            if (send_internal == TRUE)
            {
                ...
                transfer_buffer.op_code = (command->opcode & 0x1F);

                transfer_buffer.cmd_fb = 0;
                transfer_buffer.frag = 0;
                transfer_buffer.mmp = 0;

                if (BIT_TEST((command->opcode), 5))
                {
                transfer_buffer.cmd_fb = 1;
                }

                if (BIT_TEST((command->opcode), 6))
                {
                transfer_buffer.frag = 1;
                }

                if (BIT_TEST((command->opcode), 7))
                {
                transfer_buffer.mmp = 1;
                }
                ...
                if (Reveal_Queue_Reentry_Counter < REVEAL_QUEUE_MAX_REENTRIES)
                {
                    ...
                    reason_code = RevealHandleParser(&transfer_buffer,transfer_buffer.cmd_fb, TRUE, REVEAL_FULL_SERACH_INTERNAL);
                ...
                {
                    retval = FALSE;
                    REVEAL_EXCEPTION_REENTRY_CNT_OVERLOADED(transfer_buffer.api_id,transfer_buffer.op_code);
                }

                // Complete processed message, if it's not required for external routing
                #if (REVEAL_ROUTER_FEATURE != REVEAL_ROUTER_INTERNAL_ONLY)
                    if (send_external == FALSE)
                #endif
                {
                    #if (REVEAL_QUEUE_PRIORITY == ENABLED)
                        if (command->priority == TRUE)
                        {
                            if ((command >= Reveal_Queue_Priority) &&
                                (command < (Reveal_Queue_Priority + REVEAL_QUEUE_PRIORITY_SIZE)))
                            {
                                uint8 cur_index = command - Reveal_Queue_Priority;
                                Reveal_Queue_Priority_Debounce[cur_index] = REVEAL_QUEUE_IDLE;
                            }
                        }
                        else if((command >= Reveal_Queue) &&
                            (command < (Reveal_Queue + REVEAL_QUEUE_SIZE)))
                    #endif
                    {
